...
function invertColor(hex, bw) {
  ...
  // Check if hex color is valid length
  if (hex.length !== 3 && hex.length !== 6) {
    throw new Error('HEX color must be either 3 or 6 characters long.');
  }
  ...
  if (bw) {
    // Formula for luminance adapted from https://stackoverflow.com/a/3943023/112731
    return r * 0.299 + g * 0.587 + b * 0.114 > 186 ? '#000000' : '#FFFFFF';
  }

  // invert color components and pad each with zeros
  return "#" + [r, g, b].map(x => (255 - x).toString(16).padStart(2, '0')).join('');
}
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.markaspot_map = {
    ...
    transformGeoJson2heat(geojson, intensity) {
      return geojson.features.map(feature => [feature.geometry.coordinates[1], feature.geometry.coordinates[0], intensity]);
    },
